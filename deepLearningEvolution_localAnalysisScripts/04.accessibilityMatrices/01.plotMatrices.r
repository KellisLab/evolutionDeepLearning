...
saveRDS(modernArchaicFullResults, file="divergentPeaks/rds/modernArchaicFullResults.rds")
saveRDS(shuffleFullResults, file="divergentPeaks/rds/shuffleFullResults.rds")
saveRDS(homininGreatApeFullResults, file="divergentPeaks/rds/homininGreatApeFullResults.rds")

################ VIZ ###########################
modernArchaicFullResults <- readRDS("divergentPeaks/rds/modernArchaicFullResults.rds")
shuffleFullResults <- readRDS("divergentPeaks/rds/shuffleFullResults.rds")
homininGreatApeFullResults <- readRDS("divergentPeaks/rds/homininGreatApeFullResults.rds")

pdf("pdf/1a.ExampleVolcano.pdf", height=6, width=6)
ggscatter(homininGreatApeFullResults[["brain_male_embryo_105_days"]], "log2FC", "minusLogPAdj", size=1, color="Change")+
  geom_hline(yintercept=2, linetype="dashed")+geom_vline(xintercept=0) +
  geom_vline(xintercept=1, linetype="dashed")+
  geom_vline(xintercept=-1, linetype="dashed") +
  scale_color_manual(values = c("NoSignificantChange" = "darkgrey", "UpInHominins" = "#E96D4E", "UpInGreatApes" = "#244857")) +
...
measurement_data <- read.csv("rawMatrices/brain_male_embryo_105_days.txt", header=TRUE, sep="\t")
rownames(measurement_data) <- measurement_data$X
measurement_data <- subset(measurement_data, select = -c(X))
measurement_data <- measurement_data[complete.cases(measurement_data), ]
pdf("upExample.pdf", height=5, width=5)
plot_peak_boxplot("chr20.45095781.45096191", measurement_data, archaic, modern, greatApe)
dev.off()

pdf("downExample.pdf", height=5, width=5)
plot_peak_boxplot("chr5.90421186.90421635", measurement_data, archaic, modern, greatApe)
dev.off()
...
#### Figure 2C and S7 Volcanos ###
library(gridExtra)
modernArchaicFullResults <- readRDS("divergentPeaks/rds/modernArchaicFullResults.rds")
shuffleFullResults <- readRDS("divergentPeaks/rds/shuffleFullResults.rds")
homininGreatApeFullResults <- readRDS("divergentPeaks/rds/homininGreatApeFullResults.rds")
output_dir <- "volcanoPlots"
dir.create(output_dir, showWarnings = FALSE)
biosamples <- names(homininGreatApeFullResults)
all_results <- c(homininGreatApeFullResults, modernArchaicFullResults, shuffleFullResults)
max_y <- max(sapply(all_results, function(x) max(x$minusLogPAdj, na.rm = TRUE)), na.rm = TRUE)

abs_max_x <- 5
 
for (biosample in biosamples) {
  a <- ggscatter(homininGreatApeFullResults[[biosample]], x="log2FC", y="minusLogPAdj", size=0.5, color="Change") + 
      geom_hline(yintercept = 2, linetype = "dashed") +
      geom_vline(xintercept = 0) +
      geom_vline(xintercept = 1, linetype = "dashed") +
      geom_vline(xintercept = -1, linetype = "dashed") +
      scale_color_manual(values = c("NoSignificantChange" = "darkgrey", 
                                    "UpInHominins" = "#E96D4E", 
                                    "UpInGreatApes" = "#244857")) +
      ylim(0, max_y) +
      xlim(-abs_max_x, abs_max_x) +
      theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank())
  
  b <-  ggscatter(modernArchaicFullResults[[biosample]], x="log2FC", y="minusLogPAdj", size=0.5, color="Change") + 
      geom_hline(yintercept = 2, linetype = "dashed") +
      geom_vline(xintercept = 0) +
      geom_vline(xintercept = 1, linetype = "dashed") +
      geom_vline(xintercept = -1, linetype = "dashed") +
      ...
      xlim(-abs_max_x, abs_max_x) +
      theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank())
  c <- ggscatter(shuffleFullResults[[biosample]], x="log2FC", y="minusLogPAdj", size=0.5, color="darkgrey") + 
      geom_hline(yintercept = 2, linetype = "dashed") +
      geom_vline(xintercept = 0) +
      geom_vline(xintercept = 1, linetype = "dashed") +
      geom_vline(xintercept = -1, linetype = "dashed") +
      ...
      xlim(-abs_max_x, abs_max_x) +
      theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank())
  pdf(file.path(output_dir, paste0(biosample, ".pdf")), height=2.5, width = 7.5)
  grid.arrange(a,b,c,ncol=3)
  dev.off()
  #png(file.path(output_dir, paste0(biosample, ".png")), height=2.5, width = 7.5, units = "in", res = 500)
  #grid.arrange(a,b,c,ncol=3)
  #dev.off()
}
...
plot_top_peaks <- function(sorted_results, data, archaic, modern, greatApe, N = 10) {
  # Get the top N peaks
  top_peaks <- head(sorted_results$Peak, N)
  ...
  # Generate a boxplot for each peak and store it in the plot list
  for (peak in top_peaks) {
    plot_list[[peak]] <- plot_peak_boxplot(peak, data, archaic, modern, greatApe)
  }
...
summary_df$PropUp <- summary_df$UpregulatedRegions / summary_df$BackgroundRegions
summary_df$PropDown <- summary_df$DownregulatedRegions / summary_df$BackgroundRegions
